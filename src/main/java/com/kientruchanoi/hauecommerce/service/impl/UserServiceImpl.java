...
import com.kientruchanoi.hauecommerce.configuration.CustomUserDetail;
import com.kientruchanoi.hauecommerce.configuration.jwt.JwtTokenProvider;
...
import com.kientruchanoi.hauecommerce.mapper.TokenMapper;
import com.kientruchanoi.hauecommerce.mapper.UserMapper;
import com.kientruchanoi.hauecommerce.payload.EmailTo;
import com.kientruchanoi.hauecommerce.payload.repsonse.*;
import com.kientruchanoi.hauecommerce.payload.request.*;
import com.kientruchanoi.hauecommerce.repository.RefreshTokenRepository;
...
import com.kientruchanoi.hauecommerce.service.TokenService;
...
public class UserServiceImpl implements UserService {
    ...
    public ResponseEntity<BaseResponse<String>> register(RegRequest request) {
        ...
        emailService.send(EmailTo.builder()
                .sendTo(request.getEmail())
                .subject("Active HAU-Ecommerce account")
                .content("Your activation code: " + activeCode)
                .build());

        return responseFactory.success("Sucess", "Đăng ký thành công, email vui lòng kiểm tra email");
    }
...
//        CustomUserDetail userDetail = (CustomUserDetail) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        ...
        if (rt.getExpireDate().compareTo(new Date()) > 0) {
            User user = userRepository.findById(rt.getUserId())
                    .orElseThrow(() -> new APIException(HttpStatus.UNAUTHORIZED, "Token không hợp lệ"));
    ...
    public ResponseEntity<BaseResponse<String>> forgotPasswordRequest(String email) {
        ...
        emailService.send(EmailTo.builder()
                ...
                .content("Your confirm code to forgot password: " + activeCode)
                .build());
    ...
    public ResponseEntity<BaseResponse<UserResponse>> uploadAvatar(MultipartFile file) throws IOException {
        ...
        String uploadDir = "image/user/avatar/" + userDetail.getId();
        Path path = fileUploadUtils.saveFile(uploadDir, file.getBytes());
        User user = userRepository.findById(userDetail.getId())
                .orElseThrow(() -> new APIException(HttpStatus.BAD_REQUEST, "Not happened"));
        user.setAvatarUrl(path.toString().replace("\\", "/"));
        userRepository.save(user);
